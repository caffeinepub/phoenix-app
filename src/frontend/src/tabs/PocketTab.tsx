...
// ── Biometric helpers ────────────────────────────────────────────────────────
const BIOMETRIC_SUPPORTED =
  typeof window !== "undefined" && !!window.PublicKeyCredential;

async function enrollBiometric(): Promise<boolean> {
  if (!BIOMETRIC_SUPPORTED) return false;
  try {
    const cred = await navigator.credentials.create({
      publicKey: {
        challenge: crypto.getRandomValues(new Uint8Array(32)),
        rp: { name: "Phonex Pocket" },
        user: {
...
function PinLockScreen({
  ...
  const [useBiometricView, setUseBiometricView] = useState(false);
  const [biometricLoading, setBiometricLoading] = useState(false);
  const inputRefs = useRef<(HTMLInputElement | null)[]>([]);

  const biometricEnabled =
    localStorage.getItem("phonex_biometric_enabled") === "true";

  // Auto-trigger biometric on lock screen if enabled
  // biome-ignore lint/correctness/useExhaustiveDependencies: biometricEnabled intentionally omitted
  useEffect(() => {
    if (mode === "locked" && biometricEnabled && BIOMETRIC_SUPPORTED) {
      setUseBiometricView(true);
      const timer = setTimeout(() => triggerBiometric(), 500);
      return () => clearTimeout(timer);
    }
  }, [mode]);

  // Check if device supports biometrics on lock screen
  useEffect(() => {
    if (mode === "locked") {
      setShowBiometricOption(BIOMETRIC_SUPPORTED);
    }
  }, [mode]);
        ...
        <div className="flex flex-col items-center gap-5 w-full max-w-[280px]">
          ...
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Use your fingerprint or face ID to unlock Pocket instantly.
            </p>
          </div>
          <div className="flex flex-col gap-3 w-full">
            <Button
              data-ocid="pocket.biometric_enroll.button"
              onClick={async () => {
                const ok = await enrollBiometric();
                if (ok) {
                  localStorage.setItem("phonex_biometric_enabled", "true");
                }
                onUnlock();
              }}
            ...
            <Button
              data-ocid="pocket.biometric_skip.button"
              variant="ghost"
              onClick={onUnlock}
  ...
  // Biometric view on lock screen
  if (mode === "locked" && useBiometricView) {
    return (
      <div className="flex flex-col items-center justify-center h-full px-6 py-10 gap-6">
        <div className="flex flex-col items-center gap-5 w-full max-w-[280px]">
          <motion.div
            animate={biometricLoading ? { scale: [1, 1.08, 1] } : {}}
            transition={{ repeat: Number.POSITIVE_INFINITY, duration: 1.2 }}
            className="w-20 h-20 rounded-2xl phoenix-gradient flex items-center justify-center shadow-lg"
          >
          ...
          <Button
            data-ocid="pocket.biometric.button"
          ...
          <button
            type="button"
            data-ocid="pocket.pin_toggle.button"
            onClick={() => {
              setUseBiometricView(false);
              setError("");
            }}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-6 py-10 gap-6">
      ...
      >
        ...
        {/* Biometric toggle on lock screen */}
        {mode === "locked" && showBiometricOption && (
          <button
            type="button"
            data-ocid="pocket.pin_toggle.button"
            onClick={() => {
...
export default function PocketTab() {
  ...
  // Called when the lock screen triggers onUnlock from setup-confirm
  // We intercept to offer biometric enrollment if supported and not yet enrolled
  const handlePinUnlockWithBiometricCheck = useCallback(() => {
    const biometricEnabled =
      localStorage.getItem("phonex_biometric_enabled") === "true";
    if (
      BIOMETRIC_SUPPORTED &&
      !biometricEnabled &&
      (pinMode === "setup-confirm" || pinMode === "setup-enter")
    ) {
      setPinMode("biometric-enroll");
    } else {
      setPinUnlocked(true);
    }
  }, [pinMode]);
  ...
  if (!pinUnlocked) {
    return (
      <PinLockScreen
        ...
        onUnlock={
          pinMode === "setup-confirm" || pinMode === "setup-enter"
            ? handlePinUnlockWithBiometricCheck
            : handlePinUnlock
        }
        ...
        <div className="flex items-start justify-between mb-1">
          ...
          <div className="flex flex-col items-end gap-1">
            ...
            {BIOMETRIC_SUPPORTED && (
              <button
                type="button"
                data-ocid="pocket.biometric_toggle.switch"
                onClick={() => {
                  const enabled =
                    localStorage.getItem("phonex_biometric_enabled") === "true";
                  if (enabled) {
                    localStorage.removeItem("phonex_biometric_enabled");
                ...
                className={`flex items-center gap-1 text-white border-0 text-[10px] rounded-full px-2 py-0.5 transition-colors ${biometricEnabled ? "bg-green-500/50 hover:bg-green-500/70" : "bg-white/15 hover:bg-white/25"}`}
              >
                <Fingerprint className="w-3 h-3" />
                {biometricEnabled ? "Biometrics: ON" : "Biometrics: OFF"}
              </button>
            )}
          </div>
        </div>
